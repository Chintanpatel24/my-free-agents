...
def cmd_server(argv=None) -> int:
    parser = argparse.ArgumentParser(prog="my-claudecode-server", description="Start/manage the NVIDIA NIM local proxy for Claude Code.")
    ...
    if provider.needs_key and (not provider.api_key or "your-key" in provider.api_key or provider.api_key == "your-api-key"):
        ...
        print("  my-claudecode-server --set-key YOUR_NVIDIA_NIM_KEY", file=sys.stderr)
        return 2
...
def doctor() -> int:
    ...
    print("\nIf config is missing: my-claudecode-server --init")
    return 0
