...
const (
	...
	DefaultOpenrouterModel     = "google/gemini-2.0-flash-exp:free"
...
func GetProvider(values map[string]string) ProviderConfig {
	name := strings.ToUpper(getWithDefault(values, "PROVIDER", "NVIDIA_NIM"))
	if name == "OPENROUTER" {
		model := getWithDefault(values, "OPENROUTER_MODEL", DefaultOpenrouterModel)
		apiKey := values["OPENROUTER_API_KEY"]
		return ProviderConfig{
			Name:     "OPENROUTER",
			BaseURL:  strings.TrimRight(getWithDefault(values, "OPENROUTER_BASE_URL", DefaultOpenrouterBaseURL), "/"),
